...
class SpeedMetric(TrafficMetric):
    ...
    def update(self,v_data):
        new_v = set()
        #record start speed and lane of new_vehicles
        for lane in self.incoming_lanes:
            for v in v_data[lane]:
                #new entering car
                if v not in self.old_v:
                    self.v_info[v] = {'speed_list':[],'lane':None}
                    self.v_info[v]['speed_list']=[int(v_data[lane][v][traci.constants.VAR_SPEED])]
                    self.v_info[v]['lane'] = lane
                else:
                    self.v_info[v]['speed_list'].append(int(v_data[lane][v][traci.constants.VAR_SPEED]))
            new_v.update( set(v_data[lane].keys()) )
